...
module.exports.login = (req, res) => {
  ...
  User.findOne({where: {email: req.body.email}})
    .then(user => {
      ...
      if (passwordController.validatePassword(req.body.password, user.passwordHash, user.passwordSalt)) {
        const accessToken = jwtUtils.makeJwtAccessToken(user.id)
        const refreshToken = jwtUtils.makeJwtRefreshToken(user.id)
        const tokenTail = refreshToken.slice(refreshToken.length-tokenTailLength, refreshToken.length)
...
module.exports.register = (req, res) => {
  newUserValidator.validateRegisterFieldsNotEmpty(req)
    .then(()=>newUserValidator.validateRegisterFieldsData(req))
...
module.exports.refreshToken = (req, res) => {
  ...
  if (token == null || token.length < 11) return res.status(401).json({ message:"Unauthorized" })
  const tokenTail = token.slice(token.length-tokenTailLength, token.length)
  jwtUtils.jwtVerifyRefresh(token)
    .then(userid => dbToken.findToken(userid))
    .then(data => {
        if (data.refreshToken === tokenTail){
          ...
          const newRefreshTokenTail = refreshToken.slice(refreshToken.length-tokenTailLength, refreshToken.length)

          dbToken.setToken(data.user, newRefreshTokenTail)
            .then(
              () => res.status(200).json({
